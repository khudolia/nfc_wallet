...
class ProviderService extends StatefulWidget {
  ...
  /// Gets object by [context] and object [Type], or null if didn't find any instance
  ///
  /// The data from the closest [ProviderService] instance that encloses
  /// the given context.
  static T? ofOrNull<T>(BuildContext context) {
    final provider =
        context.dependOnInheritedWidgetOfExactType<_InheritedServiceProvider>();
    final dynamic data = provider!.data[T];
    return data != null ? data as T : null;
  }

  /// Gets objects by [context].
  ///
  /// The data from the all closest [ProviderService] instances that encloses
  /// the given context.
